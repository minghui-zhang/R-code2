...
onset_type <- 'freq_10_persiann' #'freq_8_chirps_ATchirps5km' #'AA_25_chirps_ATchirps5km', 'freq_10_persiann', 'AA_25_persiann'# 'Gabriel_onset'
...
CAR poly data
NOTE, couldn't get spatial data or export csv data using onset_type; this is for AA_25_gabriel
NOTE, got only percentile5 and percentile95 in the case of onset_type. if want to examine predictors like poly_area, and soy planted area, use AA_25_gabriel (uncomment the raw data and turn all the functions from ...2 into simply ...). the onset_type specific CARpoly data is for finding R2 during observation scale selection

```{r}

median_CARpoly_raw <- read.csv(paste0(first_folder,'/R-code-large-files/data_onset_', onset_type, 
                            '/median_CARpoly_', onset_type, '.csv')) #read.csv('E:/R-code2/Modeling/data/median_CARpoly_v2.csv')
percentile5_CARpoly_raw <- read.csv(paste0(first_folder,'/R-code-large-files/data_onset_', onset_type, 
                            '/percentile5_CARpoly_', onset_type, '.csv')) #read.csv('E:/R-code2/Modeling/data/percentile5_CARpoly_v2.csv')
percentile95_CARpoly_raw <- read.csv(paste0(first_folder,'/R-code-large-files/data_onset_', onset_type, 
                            '/percentile95_CARpoly_', onset_type, '.csv')) #read.csv('E:/R-code2/Modeling/data/percentile95_CARpoly_v2.csv')
...
CARpoly_raw <- create_CARpoly_raw(median_CARpoly_raw, percentile5_CARpoly_raw, percentile95_CARpoly_raw) #create_CARpoly_raw(median_CARpoly_raw, percentile5_CARpoly_raw, percentile95_CARpoly_raw)
...
# tidy by plant_stat_type
CARpoly_percentile5 <- CARpoly_tidy
CARpoly_percentile5$plant <- CARpoly_tidy$plant_percentile5
CARpoly_percentile5$plant_stat_type <- rep("percentile5", nrow(CARpoly_percentile5))

CARpoly_percentile50 <- CARpoly_tidy
CARpoly_percentile50$plant <- CARpoly_tidy$plant_median
CARpoly_percentile50$plant_stat_type <- rep("median", nrow(CARpoly_percentile50))

CARpoly_percentile95 <- CARpoly_tidy
CARpoly_percentile95$plant <- CARpoly_tidy$plant_median
CARpoly_percentile95$plant_stat_type <- rep("percentile95", nrow(CARpoly_percentile95))

CARpoly_tidy <- rbind(CARpoly_percentile5, CARpoly_percentile50, CARpoly_percentile95)
...
note, used plant_stat_type as a predictor. can use onset_type (not just AA_Gabriel) for muni and CARpoly scales, but may need to extract from GEE using Extract_data_v2
...
model_CARpoly <- lm(plant ~ onset + year_index + latitude + longitude + intensity + plant_stat_type, data = CARpoly_tidy)
model_cell <- lm(plant ~ onset + year + lat + lon + intensity + plant_stat_type, data = cell_sf_tidy)
model_muni <- lm(plant ~ onset + year + lat + lon + intensity + plant_stat_type, data = muni_sf_tidy)

R2_CARpoly <- summary(model_CARpoly)$r.squared
R2_cell <- summary(model_cell)$r.squared
R2_muni <- summary(model_muni)$r.squared

print(paste('R2 for (CARpoly, cell, and muni):', R2_CARpoly, R2_cell, R2_muni)) #  
...
# import CARpoly data for one onset cell
# import data -----------------------------------------------------------------------------------------------
...
layername_CARpoly_shp <- paste0('median_CARpoly_SHP_', onset_type)
...
CARpoly_sf <- CARpoly_sf[!is.na(CARpoly_sf$SC_plant) & !is.na(CARpoly_sf$DC_plant),]
...
# percentile25_CARpoly <- percentile25_CARpoly_raw %>% filter(year > 0)
# 
# # create tidy datasets
# 
# CARpoly_tidy <- tidy_combine_muni(median_CARpoly, percentile5_CARpoly, percentile25_CARpoly)
# 
...
# CARpoly_sf_percentile5$SC_plant <- percentile5_CARpoly$single_plant
# CARpolysf_percentile5$DC_plant <- percentile5_CARpoly$double_plant
# CARpoly_sf_percentile5$plant_stat_type <- rep("percentile5", nrow(CARpoly_sf_percentile5))
# 
# CARpoly_sf_percentile25 <- CARpoly_sf
# CARpoly_sf_percentile25$SC_plant <- percentile25_CARpoly$single_plant
# CARpoly_sf_percentile25$DC_plant <- percentile25_CARpoly$double_plant
...
ggplot(CARpoly_sf_tidy_year) +
  geom_sf(aes(fill = DC_plant)) + # , colour = NA) +
  scale_fill_viridis(limits = c(30, 150)) +
  ggtitle(paste("CARpoly median planting date", year_oi, intensity_oi, percentile_oi)) +
  #geom_polygon(data = MT_outline, aes(x = long, y = lat), color = "black", alpha = 0, linetype = 1) +
  theme_bw()
```
...
#                                                 onset:lat + onset:lon + prev_onset, lower = ~ 0+onset), direction = 'forward', trace = FALSE)
...
new.predictors <- c('year_index', 'intensity', 'lat', 'lon', 'region', 'plant_stat_type', 'onset:intensity', 'onset:plant_stat_type', 'onset:lat', 'onset:lon', 'ann_rain', 'prev_onset') 

selected_model_adjR2 <- produce_model_stepwise(data = cell_sf_tidy, initial.predictors = c('onset'), y.var = 'plant',
                       new.predictors = new.predictors, mask.soy.area = TRUE, max.predictors = 5)
...
selected_OLS_model <- lm(plant ~ onset + plant_stat_type + intensity + year_index + region + onset:plant_stat_type + onset:intensity + ann_rain + prev_onset, data = cell_sf_tidy)
...
for (grid_size in seq(0.15, 1, 0.10)) { # 0.25 to 1.75, , seq(0.15, 1, 0.10)
    for (lat_offset in seq(0, 0.75, by = 0.18)) { # 0 to 1.5, DON'T DO BY = 0.25
      for (lon_offset in seq(0, 0.75, by = 0.18)) { # 0 to 1.5
...
saveRDS(results_summary, paste0("OLS_spatial_sampling_decision_", onset_type, ".rds"))
```
...
test_elimyear <- function(elim_year, cell_df, cell_sf, f, model_type, grid_size, plant_stat) {
  ...
  if (model_type == "ols") {
    ...
    for (lat_offset in seq(0, 0.75, by = 0.2)) { # 0 to 1.5
      for (lon_offset in seq(0, 0.75, by = 0.2)) { # 0 to 1.5
...
do_elimcell <- TRUE
do_elimyear <- TRUE
cell_reducer <- 100 # the number of cells by this when NOT sampling data and doing elimcell
...
chosen_percentile <- "percentile25"
chosen_grid_size <- 0.25
chosen_intensity <- "DC"
...
imported_prediction_error <- readRDS('prediction_error_results_freq_8_chirps_ATchirps5km_DC_percentile25.rds')
...
for (intensity in c("DC", "SC")) {
  for (percentile in c("percentile5", "percentile25", "median", "percentile75", "percentile95")) {
    ...
    plot(x = as.numeric(data_subset$fitted.values), y = as.numeric(data_subset$residuals), 
         ...
         xlab = 'de-meaned fitted value', ylab = 'residual')
    abline(h = 0, col = 'red')
...
for (intensity in c("DC", "SC")) {
  for (percentile in c("percentile5", "percentile25", "median", "percentile75", "percentile95")) {
    ...
    # plot fe
    year <- 2004 # all years will be the same for fixed effects
...
for (intensity in c("SC", "DC")) {
  for (percentile in c('percentile5', 'percentile25', 'median', 'percentile75', 'percentile95')) {
    ...
    ols_prediction_map <- ggplot(valid_sf_elimyear) +
      ...
      scale_fill_viridis() + # limits = c(-40, 40)
...
# save data_bootstrap_coefs (every bootstrapped coef for each planting dataset)
...
names(onset_coefs_list) <- c('SC_percentile5', 'SC_percentile25', 'SC_median', 
                             'SC_percentile75', 'SC_percentile95', 
                             'DC_percentile5', 'DC_percentile25', 'DC_median',
                             'DC_percentile75', 'DC_percentile95')
names(year_coefs_list) <- c('SC_percentile5', 'SC_percentile25', 'SC_median', 
                            'SC_percentile75', 'SC_percentile95',
                            'DC_percentile5', 'DC_percentile25', 'DC_median',
                            'DC_percentile75', 'DC_percentile95')

# compare coefs across intensities and groups
num_samples_ttest <- 100 # the number of samples to take in the t test. more samples -> lower p value. total samples: 10000
diff_intensities_median <- t.test(onset_coefs_list[['SC_median']][0:num_samples_ttest], 
                                  onset_coefs_list[['DC_median']][0:num_samples_ttest], 
                                 alternative = "two.sided", var.equal = FALSE)$p.value
diff_intensities_percentile25 <- t.test(onset_coefs_list[['SC_percentile25']][0:num_samples_ttest], 
                                        onset_coefs_list[['DC_percentile25']][0:num_samples_ttest], 
                                 alternative = "two.sided", var.equal = FALSE)$p.value
diff_intensities_percentile5 <- t.test(onset_coefs_list[['SC_percentile5']][0:num_samples_ttest], 
                                       onset_coefs_list[['DC_percentile5']][0:num_samples_ttest], 
                                 alternative = "two.sided", var.equal = FALSE)$p.value
diff_intensities_percentile75 <- t.test(onset_coefs_list[['SC_percentile75']][0:num_samples_ttest], 
                                        onset_coefs_list[['DC_percentile75']][0:num_samples_ttest], 
                                 alternative = "two.sided", var.equal = FALSE)$p.value
diff_intensities_percentile95 <- t.test(onset_coefs_list[['SC_percentile95']][0:num_samples_ttest], 
                                        onset_coefs_list[['DC_percentile95']][0:num_samples_ttest], 
                                 alternative = "two.sided", var.equal = FALSE)$p.value

diff_percentile5_25_SC <- t.test(onset_coefs_list[['SC_percentile5']][0:num_samples_ttest], 
                                 onset_coefs_list[['SC_percentile25']][0:num_samples_ttest], 
                                 alternative = "two.sided", var.equal = FALSE)$p.value
diff_percentile5_25_DC <- t.test(onset_coefs_list[['DC_percentile5']][0:num_samples_ttest], 
                                 onset_coefs_list[['DC_percentile25']][0:num_samples_ttest], 
                                 alternative = "two.sided", var.equal = FALSE)$p.value

diff_percentile25_50_SC <- t.test(onset_coefs_list[['SC_percentile25']][0:num_samples_ttest], 
                                  onset_coefs_list[['SC_median']][0:num_samples_ttest], 
                                 alternative = "two.sided", var.equal = FALSE)$p.value
diff_percentile25_50_DC <- t.test(onset_coefs_list[['DC_percentile25']][0:num_samples_ttest], 
                                  onset_coefs_list[['DC_median']][0:num_samples_ttest], 
                                 alternative = "two.sided", var.equal = FALSE)$p.value

diff_percentile50_75_SC <- t.test(onset_coefs_list[['SC_median']][0:num_samples_ttest], 
                                  onset_coefs_list[['SC_percentile75']][0:num_samples_ttest], 
                                 alternative = "two.sided", var.equal = FALSE)$p.value
diff_percentile50_75_DC <- t.test(onset_coefs_list[['DC_median']][0:num_samples_ttest], 
                                  onset_coefs_list[['DC_percentile75']][0:num_samples_ttest], 
                                 alternative = "two.sided", var.equal = FALSE)$p.value

diff_percentile75_95_SC <- t.test(onset_coefs_list[['SC_percentile75']][0:num_samples_ttest], 
                                  onset_coefs_list[['SC_percentile95']][0:num_samples_ttest], 
                                 alternative = "two.sided", var.equal = FALSE)$p.value
diff_percentile75_95_DC <- t.test(onset_coefs_list[['DC_percentile75']][0:num_samples_ttest], 
                                  onset_coefs_list[['DC_percentile95']][0:num_samples_ttest], 
                                 alternative = "two.sided", var.equal = FALSE)$p.value

print('p values comparing intensities: (for percentile95, to percentile5)')
...
print('p values comparing percentile5 and percentile25 (for SC, then DC):')
print(paste(diff_percentile5_25_SC, diff_percentile5_25_DC))

print('p values comparing percentile25 and median (for SC, then DC):')
print(paste(diff_percentile25_50_SC, diff_percentile25_50_DC))

print('p values comparing percentile50 and percentile75 (for SC, then DC):')
print(paste(diff_percentile50_75_SC, diff_percentile50_75_DC))

print('p values comparing percentile75 and percentile95 (for SC, then DC):')
...
onset_coef_plot <- ggplot(final_bootstrap_coefs) +
  ...
  geom_errorbar(aes(ymin=onset_coef-onset_sderr, ymax=onset_coef+onset_sderr, x = percentile)) +
...
year_coef_plot <- ggplot(final_bootstrap_coefs) +
  ...
  geom_errorbar(aes(ymin=year_coef-year_sderr, ymax=year_coef+year_sderr, x = percentile)) +
  facet_wrap(~ intensity) +
...
# SAVED RESULTS
saveRDS(final_bootstrap_coefs, paste0("FE_bootstrapped_coefs_", onset_type, ".rds"))
```

```{r}

coefs_onset_oi <- readRDS(paste0("FE_bootstrapped_coefs_", onset_type, ".rds"))
