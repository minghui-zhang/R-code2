...
```{r}
onset_oi <- 'freq_10_persiann' #'freq_10_persiann'
...
onset_change_vulnerable <- 10
onset_change_not_vulnerable <- 1
year_change <- 10 # number of years after year_oi for prediction

cell_df <- cell_sf_tidy
st_geometry(cell_df) <- NULL

coefs_freq_10_persiann <- readRDS("FE_bootstrapped_coefs_freq_10_persiann.rds")
coefs_freq_10_persiann$percentile <- as.character(coefs_freq_10_persiann$percentile)
coefs_freq_10_persiann[which(coefs_freq_10_persiann[,"percentile"] == "median"), "percentile"] <- "percentile50"
...
for (intensity in c('SC', 'DC')) {
  for (region in c('vulnerable', 'not_vulnerable')) {
    ...
    if (region == 'vulnerable') {
      cell_df_subset <- cell_df[cell_df$year == year_oi & cell_df$intensity == intensity & cell_df$lon > region_cutoff,] # east
      onset_change <- onset_change_vulnerable
    }
    
    if (region == 'not_vulnerable') {
      cell_df_subset <- cell_df[cell_df$year == year_oi & cell_df$intensity == intensity & cell_df$lon <= region_cutoff,] # west
    ...
    for (percentile in c('percentile5', 'percentile25', 'percentile50', 'percentile75', 'percentile95')) {
      ...
      # predictions
      plant_pred_onset_change <- mean_observed + onset_change*coefs_freq_10_persiann[coefs_freq_10_persiann$intensity == intensity &
                                                                                       coefs_freq_10_persiann$percentile == percentile, "onset_coef"]
      plant_pred_year_change <- mean_observed + year_change*coefs_freq_10_persiann[coefs_freq_10_persiann$intensity == intensity &
                                                                                       coefs_freq_10_persiann$percentile == percentile, "year_coef"]
      plant_pred_onset_year_change <- mean_observed + onset_change*coefs_freq_10_persiann[coefs_freq_10_persiann$intensity == intensity &
                                                                                       coefs_freq_10_persiann$percentile == percentile, "onset_coef"] +
                                                      year_change*coefs_freq_10_persiann[coefs_freq_10_persiann$intensity == intensity &
                                                                                       coefs_freq_10_persiann$percentile == percentile, "year_coef"]
