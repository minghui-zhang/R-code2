# functions that can run FE, RE and ME models as well as OLS

run_any_model <- function(full_data, model_type, predictors, ME_randomEffects, FE_RE_effect,
                    y.var, plant_stat,
  ...
  # model_type: OLS, FE, RE or ME
  # predictors: 'regular' predictors
  # ME_randomEffects: the random effects in a mixed effect model, to be done like (1|variablename)
  # FE_RE_effect: the 'unit' to use as the group. cell_ID, or cell_ID and year. must have only one data point per year for each group.
  ...
  sampled_data_sf <- sampled_data
  
  # map of the input data used (for specific year and intensity)
  if (plot_samples) {
    print(plot_cell_onset(year_oi, sampled_data_sf))
    print(plot_cell_plant(year_oi, sampled_data_sf, "DC"))
  }
  
  # define effects if FE or RE
  if (model_type %in% c("FE", "ME")) {
    sampled_data <- pdata.frame(sampled_data,index = FE_RE_effect)
  }
  
  # define the formula
  formula.string.regular <- paste(y.var, paste(predictors, collapse = " + "), sep = " ~ ")
  
  if (model_type %in% c("OLS", "FE", "RE")) {f <- as.formula(formula.string.regular)}
  
  # mixed effects: define additional random effect terms
  if (model_type == "ME") {
  ...
  if (model_type == "OLS") {model = lm(f, data=sampled_data)}
  if (model_type == "FE") {model = plm(f, data = sampled_data, model = "within")}
  if (model_type == "RE") {model = plm(f, data = sampled_data, model = "random")}
  if (model_type == "ME") {model = lmer(f, data = sampled_data)}
  
  sampled_data_sf$residuals <- residuals(model)
  sampled_data_sf$fitted.values <- fitted.values(model)
  
  # map of residuals
  if (plot_model_evals) {print(plot_cell_residuals(year_oi, sampled_data_sf, "DC"))}
  ...
  # 2. percent data used
  percent_data_used <- 100*nrow(sampled_data_sf)/nrow(full_data)
  
  # 3. R2
  SST <- sum((sampled_data_sf$plant - mean(sampled_data_sf$plant))^2)
  SSE <- sum((sampled_data_sf$residuals - mean(sampled_data_sf$residuals))^2)
  R2 <- 1 - SSE/SST
  
  # 4. exogeneity plots (residual vs predictors)
  if (plot_model_evals) {
    plot(sampled_data_sf$onset, sampled_data_sf$residuals, main = "onset vs residual (exogeneity)")
    #plot(sampled_data_sf$lat, sampled_data_sf$residuals, main = "latitude vs residual (exogeneity)")
    
    plot(sampled_data_sf$year, sampled_data_sf$residuals, main = "year vs residual (exogeneity)") 
    abline(h = 0)
  }
  ...
  to_autocorrelation <- sampled_data_sf[sampled_data_sf$year == year_oi & sampled_data_sf$intensity == chosen_intensity, ]
  ...
  if (plot_model_evals) {
    ...
    plot(sampled_data_sf$plant, sampled_data_sf$fitted.values, 
         main = "fitted value vs actual value of plant", ylab = "fitted.values")
    abline(h = mean(sampled_data_sf$fitted.values), col = "blue")
    abline(v = mean(sampled_data_sf$plant), col = "blue")
    abline(0,1, col = "gray", lwd = 3)
  }
  
  # 7. multicollinearity: look at max absolute correlation between predictors
  predictors <- sampled_data_sf[,c("onset", "year")]
  ...
  if (do_elim_year) { # set do_elim_year to FALSE if using year as fixed effect
    ...
    for (year in 2004:2014) {
      ...
      result <- run_any_model_for_prediction_elimyear(sampled_data_sf, year, TRUE, f, model_type)
      prediction_results_elimyear <- rbind(prediction_results_elimyear, result)
      
      print('elimyear prediction')
      print(prediction_results_elimyear)
    }
  ...
  # prediction_results_elimlocation <- run_any_model_for_prediction_elimlocation(full_data = sampled_data_sf,
  #                                                                          formula = f,
...
# cross_validation by location
run_any_model_for_prediction_elimlocation <- function(full_data, formula, year_for_location, year_oi, plot_model_evals, model_type) {
  ...
  for (row_index in 1:nrow(data_in_year)) {
    ...
    print('formula')
    print(formula)
    print(plm(formula, data = train_data, model = "within"))
    if (model_type == "OLS") {model = lm(formula, data=train_data)}
    if (model_type == "FE") {model = plm(formula, data = train_data, model = "within")}
    if (model_type == "RE") {model = plm(formula, data = train_data, model = "random")}
    if (model_type == "ME") {model = lmer(formula, data = train_data)}
...
# cross validation for new year
run_any_model_for_prediction_elimyear <- function(full_data, year_to_elim, elim_year, formula, model_type) {
