...
precip_data <- 'Xavier'
onset_def_type <- 'AA'
thres <- '25' # keep as string; some will have two or four thres
onset_scale <- '25km_Gabriel'
...
lat_offsets <- seq(0, 1.5, by = 0.25) 
lon_offsets <- seq(0, 1.5, by = 0.25) 
    ...
    for (plant_stat_type in plant_stat_types) {
      for (crop_intensity in crop_intensities) {
        ...
        sampled <- get_sampled_data(full_data = cell_sf_tidy_intensity, plant_stat = plant_stat_type, grid_size = grid_size,
                         lat_offset = lat_offset, lon_offset = lon_offset, agg_scheme = FALSE, plot_samples = FALSE, year_oi = 2012)
        ...
        nb <- poly2nb(sampled_withGeom)
        
        resnb <- sapply(nb, function(x) mean(sampled_withGeom$residual[x]))
        lw <- nb2listw(nb, zero.policy = TRUE)
        
        moran_residual <- moran.mc(sampled_withGeom$residual, lw, 999, zero.policy = TRUE)
        moran_i_residual <- moran_residual$statistic
        moran_i_pval_residual <- moran_residual$p.value
        
        moran_onset <- moran.mc(sampled_withGeom$onset, lw, 999, zero.policy = TRUE)
        moran_i_onset <- moran_onset$statistic
        moran_i_pval_onset <- moran_onset$p.value
        
        moran_plant <- moran.mc(sampled_withGeom$plant, lw, 999, zero.policy = TRUE)
